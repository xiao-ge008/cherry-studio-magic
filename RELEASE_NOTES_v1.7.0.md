...
本版本大幅优化了组件调用体验，现在可以在任意聊天中像调用 MCP 工具一样轻松使用组件！

- **无缝集成**：组件不再局限于特定模式，支持在所有对话中按需调用。
- **MCP 风格体验**：统一的工具调用接口，让组件使用更加自然流畅。
- **智能触发**：基于上下文智能识别何时调用组件，提升交互效率。
