...
import { generateComponentSkillPrompt, validateComponentConfig } from '@renderer/utils/componentConfig'
...
export class ComponentService {
  ...
  generateComponentPrompt(
    ...
    try {
      ...
      // 如果配置中已经保存了自定义 Markdown 提示词，并且当前请求的是 markdown 格式
      // 则优先使用用户自定义的提示词。
      const maybeCustomPrompt = (config as any).skillPromptMarkdown
      if (options?.format === 'markdown' && typeof maybeCustomPrompt === 'string' && maybeCustomPrompt.trim().length > 0) {
        logger.info('Using custom component skill prompt', { data: id })
        return maybeCustomPrompt
      }

      const prompt = generateComponentSkillPrompt(config, {
  ...
  /**
   * 根据指定组件 ID 列表生成组件技能的 AI 提示词
   * 主要用于根据 assistant 中选择的组件
   */
  generateComponentsPromptByIds(
    ids: string[],
    options?: {
      includeExamples?: boolean
      includeParameterDetails?: boolean
    ...
    try {
      ...
      const components = ids
        .map((id) => this.getComponentConfig(id))
        .filter((c): c is ComponentConfig => !!c && c.enabled)
        ...
        const settings = this.getComponentSettings()
        const customPromptMarkdown = settings.componentsPromptMarkdown?.trim()

        let prompt = ''

      if (options?.format === 'markdown') {
        prompt += `# ${isZhCN ? '组件能力总纲' : 'Components Capabilities Overview'}\n\n`
        prompt += `${
          isZhCN
            ? '你现在可以使用以下专业组件来增强你的能力。当用户的需求与这些组件的功能相匹配时，请主动且智能地使用对应的组件。每个组件都有其特定的应用场景和触发条件，请仔细阅读并理解：'
            : "You now have access to the following professional components to enhance your capabilities. When user requests match these components' functionalities, please proactively and intelligently use the corresponding components. Each component has its specific use cases and trigger conditions. Please read carefully:"
        }\n\n`
      } else {
        prompt += `${isZhCN ? '组件能力总纲' : 'Components Capabilities Overview'}\n\n`
        prompt += `${
          isZhCN
            ? '你现在可以使用以下专业组件来增强你的能力。当用户的需求与这些组件的功能相匹配时，请主动且智能地使用对应的组件。'
            : "You now have access to the following professional components to enhance your capabilities. When user requests match these components' functionalities, please use them intelligently."
        }\n\n`
      }
        ...
        components.forEach((component, index) => {
          ...
          const componentPrompt =
            this.generateComponentPrompt(component.id, {
              includeExamples: options?.includeExamples ?? true,
              includeParameterDetails: options?.includeParameterDetails ?? true,
              language,
        ...
        // 如果在组件设置中配置了自定义「组件提示词」，
        // 则在 Markdown + 中文模式下，使用该内容作为头部大纲重新生成整体提示词。
        if (options?.format === 'markdown' && isZhCN && customPromptMarkdown) {
          let rebuilt = customPromptMarkdown.trimEnd() + '\n\n'
          ...
          components.forEach((component, index) => {
            ...
            rebuilt += componentPrompt
          })

          prompt = rebuilt
        }
 
        logger.info('Generated AI prompt for selected components', {
      ...
      const err = error instanceof Error ? error : new Error(String(error))
      logger.error('Failed to generate selected components prompt', err)
      return ''
    }
  }
