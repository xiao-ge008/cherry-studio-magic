...
export async function fetchChatCompletion({
  ...
  // 🔧 关键修复：将包含组件提示词的 system prompt 临时设置给 assistant
  // 这样无论走 Modern SDK 还是 Legacy Client 路径，组件提示词都能正确传递
  const originalPrompt = assistant.prompt
  if (aiSdkParams.system) {
    assistant.prompt = aiSdkParams.system
  }
...
export async function fetchMessagesSummary({ messages, assistant }: { messages: Message[]; assistant: Assistant }) {
  ...
  // Use assistant's model first, then getQuickModel, then default
  // This prevents using remote models for CLI-based assistants
