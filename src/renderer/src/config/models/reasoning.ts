...
export function isReasoningModel(model?: Model): boolean {
  ...
  if (modelId === 'gemini-cli' || modelId === 'qwen-cli') {
    return true
  }
...
export const THINKING_TOKEN_MAP: Record<string, { min: number; max: number }> = {
  // CLI models (use same limits as their base models)
  'gemini-cli$': { min: 0, max: 24576 }, // Same as gemini-flash
  'qwen-cli$': { min: 0, max: 81_920 }, // Same as qwen-plus
