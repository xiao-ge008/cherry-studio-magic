...
const Markdown: FC<Props> = ({ block, postProcess }) => {
  ...
  const messageContent = useMemo(() => {
    ...
    let text = removeSvgEmptyLines(processLatexBrackets(displayedContent))

    // For non-thinking blocks, transform <thinking>...</thinking> sections into
    // a collapsible <details> block so users can expand to see the chain-of-thought.
    if (block.type !== MessageBlockType.THINKING) {
      if (/<thinking\b[^>]*>[\s\S]*?<\/thinking>/i.test(text)) {
        text = text
          .replace(/<thinking\b[^>]*>/gi, '<details class="cs-thinking-block"><summary>已深度思考（点击展开）</summary>\n')
          .replace(/<\/thinking>/gi, '\n</details>')
          .trimStart()
      }
    }
  ...
  const comfyUIComponents = useMemo(() => {
    try {
      // 只使用已启用的 ComfyUI 组件
      return componentService.getComfyUIComponents().filter((component) => component.enabled)
    } catch (error) {
  ...
  const components = useMemo(() => {
    // 获取JS组件列表
    const jsComponents = componentService.getJSComponents().filter((component) => component.enabled)
    ...
    const baseComponents = {
      ...
      p: (props) => {
        ...
        // 检查是否包含 ComfyUI 组件
        const hasComfyUIComponent = props?.node?.children?.some(
          (child: any) =>
            child.type === 'element' &&
            child.tagName &&
            typeof child.tagName === 'string' &&
            child.tagName.startsWith('comfyui-')
        )

        if (hasImage || hasOptionsComponent || hasComfyUIComponent) return <div {...props} />
        return <p {...props} />
      },
    ...
    comfyUIComponents.forEach((component) => {
      ...
      baseComponents[tagName] = (props: any) => {
        return <ComfyUIComponent componentName={component.componentName} {...props} />
      }
    ...
    jsComponents.forEach((component) => {
      ...
      baseComponents[tagName] = (props: any) => {
        return <JSComponent name={component.componentName} {...props} />
      }
