...
import ComponentToolsButton, { ComponentToolsButtonRef } from './ComponentToolsButton'
...
const InputbarTools = forwardRef<InputbarToolsRef, InputbarToolsProps>(
  (
    {
      assistantId,
      model,
  ...
  const componentToolsButtonRef = useRef<ComponentToolsButtonRef>(null)
  ...
  const handleComponentSelect = useCallback(
    (componentIds: string[]) => {
      updateAssistant({ component_ids: componentIds })
      setSelectedComponentIds(componentIds)
    },
    [setSelectedComponentIds, updateAssistant]
  )
  ...
  const getQuickPanelMenuImpl = (params: { text: string; translate: () => void }): QuickPanelListItem[] => {
    ...
    return [
      ...
      },
      {
        ...
        isMenu: true,
        action: () => {
          componentToolsButtonRef.current?.openQuickPanel()
        }
      },
      {
  ...
  const toolButtons = useMemo<ToolButtonConfig[]>(() => {
    return [
      ...
      },
      {
        ...
        label: t('settings.components.select_components'),
        component: (
          <ComponentToolsButton
            ref={componentToolsButtonRef}
  ...
  const visibleTools = useMemo(() => {
    // �Ѵ�����/���صĹ��߰�պ���
    const specifiedVisible = toolOrder.visible
      .map((key) => toolButtons.find((tool) => tool.key === key))
      .filter((tool): tool is ToolButtonConfig => !!tool)
      .map((tool) => ({ ...tool, visible: true }))

    const specifiedHiddenKeys = new Set(toolOrder.hidden)
    const knownKeys = new Set([...toolOrder.visible, ...toolOrder.hidden])

    // �������µĹ��ߣ������� toolOrder ��ʱ����Ĭ�Ϸ���
    const extraVisible = toolButtons
      .filter((tool) => !knownKeys.has(tool.key))
      .map((tool) => ({ ...tool, visible: true }))

    // ��ֹ�ظ����� key
    const merged: ToolButtonConfig[] = [...specifiedVisible]
    extraVisible.forEach((tool) => {
      if (!specifiedHiddenKeys.has(tool.key) && !merged.some((t) => t.key === tool.key)) {
        merged.push(tool)
      }
    })
