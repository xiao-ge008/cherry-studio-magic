...
import { ArrowLeft, Edit, Eye, EyeOff, Plus, Settings, Trash2 } from 'lucide-react'
...
const ComfyUIConfigurePage: FC = () => {
  ...
  const [editingValueNodeId, setEditingValueNodeId] = useState<string | null>(null)
  const [editingValueInputField, setEditingValueInputField] = useState<string | null>(null)
  ...
  // 打开默认值编辑模态框
  const handleOpenEditValueModal = (nodeId: string, inputField: string, currentValue: any) => {
    setEditingValueNodeId(nodeId)
    setEditingValueInputField(inputField)
  ...
  const handleCloseEditValueModal = () => {
    ...
    setEditingValueNodeId(null)
    setEditingValueInputField(null)
    setEditingValue(null)
  }

  // 保存修改后的默认值
  const handleSaveDefaultValue = (newValue: any) => {
    if (!editingValueNodeId || !editingValueInputField || !component) return

    // 深拷贝组件配置以避免直接修改
    const updatedComponent = JSON.parse(JSON.stringify(component))

    // 更新工作流模板中的默认值
    if (updatedComponent.workflowTemplate[editingValueNodeId]) {
      updatedComponent.workflowTemplate[editingValueNodeId].inputs[editingValueInputField] = newValue
    }
    ...
    // 重新解析节点以更新 UI
    const analysisResult = WorkflowParser.parseWorkflow(updatedComponent.workflowTemplate)
    setNodes(analysisResult.nodes)

    setHasChanges(true)
    handleCloseEditValueModal()
    window.toast.success('默认值已更新')
  }
    ...
    <SettingContainer>
      <VStack gap="24px">
                                  ...
                                  <HStack justifyContent="space-between" alignItems="center">
                                    ...
                                    <div className="flex gap-2">
                                      <Button
                                        ...
                                        variant="light"
                                        onPress={() =>
                                          handleOpenEditValueModal(selectedNode.id, inputField, currentValue)
                                        }
                                        title="修改默认值">
        ...
        <EditDefaultValueModal
          ...
          onClose={handleCloseEditValueModal}
          onSave={handleSaveDefaultValue}
          nodeId={editingValueNodeId}
          inputField={editingValueInputField}
...
interface EditDefaultValueModalProps {
  isOpen: boolean
  onClose: () => void
...
const EditDefaultValueModal: FC<EditDefaultValueModalProps> = ({
  isOpen,
  onClose,
  onSave,
  nodeId,
  ...
  const [type, setType] = useState<ComponentParameter['type']>('string')
  ...
  const handleSave = () => {
    ...
    try {
      if (type === 'number') {
        ...
        if (isNaN(finalValue)) throw new Error('无效的数字')
      ...
      window.toast.error(`格式错误: ${(e as Error).message}`)
      return
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <ModalContent>
        <ModalHeader>
          修改默认值
          <span className="ml-2 font-normal text-default-500 text-sm">
            {nodeId} → {inputField}
          </span>
        </ModalHeader>
        <ModalBody>
          <VStack gap="16px">
            <Select
              ...
              <SelectItem key="number">数字</SelectItem>
              <SelectItem key="boolean">布尔值</SelectItem>
              <SelectItem key="json">JSON / 对象</SelectItem>
            </Select>

            {type === 'boolean' ? (
              <Switch isSelected={value === 'true'} onValueChange={(v) => setValue(String(v))}>
                {value === 'true' ? 'True' : 'False'}
              </Switch>
            ) : type === 'json' || (type === 'string' && value.length > 50) ? (
              <Textarea
                label="值"
                value={value}
                ...
                minRows={4}
                variant="bordered"
                classNames={{ inputWrapper: baseInputWrapperClass, input: baseFieldInputClass }}
              />
            ) : (
              <Input
                label="值"
