...
import { ComfyUIComponentConfig, ComponentConfig, JSComponentConfig } from '@renderer/types/component'
import { Code, Cpu, Download, Edit, Package, Plus, Settings, Sparkles, Trash2 } from 'lucide-react'
import { FC, useEffect, useState } from 'react'
import { useTranslation } from 'react-i18next'
import { useNavigate } from 'react-router-dom'

import { SettingContainer } from '..'
import GenerationPromptSettings from './GenerationPromptSettings'
import GlobalPromptSettings from './GlobalPromptSettings'
...
const ComponentSettings: FC = () => {
  ...
  const loadComponents = async () => {
    try {
      ...
      logger.error('Failed to load components:', err)
    }
  }
  ...
  const handleGenerateComponentMD = (component: ComponentConfig) => {
    // 从 Redux store 重新读取最新的组件配置（包含最新保存的 skillPromptMarkdown）
    const latestComponent = componentService.getComponentConfig(component.id)
    if (!latestComponent) {
      window.toast.error('组件不存在')
      return
    }

    const maybeComponent = latestComponent as any
    const slugValue =
      typeof maybeComponent?.componentName === 'string' && maybeComponent.componentName.trim().length > 0
        ? maybeComponent.componentName.trim()
        : latestComponent.id

    let displayName = latestComponent.name
    if (latestComponent.category === 'javascript') {
      displayName = `js-${slugValue}`
    } else if (latestComponent.category === 'comfyui') {
      displayName = `comfyui-${slugValue}`
    } else if (latestComponent.category === 'media') {
      displayName = `audio-${slugValue}`
    } else if (latestComponent.category === 'interaction') {
      displayName = `option-${slugValue}`
    }

    setSelectedComponent({ ...latestComponent, name: displayName })
    onMDOpen()
  }
    ...
    <SettingContainer>
      <VStack gap="24px">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          {/* 新增 ComfyUI 组件卡片 */}
          <Card className="flex h-[280px] cursor-pointer border-2 border-primary-300 border-dashed bg-primary-50/30 transition-all duration-200 hover:scale-[1.01] hover:shadow-md">
          ...
          {/* 新增 JS 组件卡片 */}
          <Card className="flex h-[280px] cursor-pointer border-2 border-secondary-300 border-dashed bg-secondary-50/30 transition-all duration-200 hover:scale-[1.01] hover:shadow-md">
            <CardBody className="flex flex-col items-center justify-center p-6 text-center">
              <VStack gap="12px" alignItems="center">
