...
export function getReasoningEffort(assistant: Assistant, model: Model): ReasoningEffortOptionalParams {
  ...
  if (!reasoningEffort) {
    ...
    // gemini - only restrict thinking when it's explicitly turned OFF
    if (isSupportedThinkingTokenGeminiModel(model)) {
      // Don't set any restrictions when thinking is enabled
      // The thinking_budget:0 restriction should only apply when reasoningEffort is undefined
